...
public class QuestionDetailActivity extends Activity implements TopBarView.onTitleBarClickListener,View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        public_choice.setView(QuestionDetailActivity.this);//设置视图
    ...
    public void publishQuestion() {
        ...
        if(TextUtils.isEmpty(question_title.getText().toString().trim())
            ...
            Toast.makeText(QuestionDetailActivity.this,ConstantValue.TXT_EMPTY,Toast.LENGTH_SHORT).show();
            return;
        }

        String uerID = PrefUtils.getUserID(QuestionDetailActivity.this.getBaseContext());
        String token = PrefUtils.getToken(QuestionDetailActivity.this.getBaseContext());
        ...
        MyHttpUtils.build()//构建myhttputils
                ...
                .onExecuteByPost(new StringCallBack() {//开始执行，并有一个回调（异步的哦---->直接可以更新ui）
                    ...
                    public void onSucceed(String result) {//请求成功之后会调用这个方法----显示结果
                        ...
                        try {
                            ...
                            if (ConstantValue.SUCCESS_STATUS.equals(status)) {
                                //TODO 保存成功后的处理
                                Toast.makeText(QuestionDetailActivity.this,"问题发布成功",Toast.LENGTH_SHORT).show();
                                QuestionDetailActivity.this.finish();
                            } else if (ConstantValue.ERROR_STATUS.equals(status)) {
                                //error
                                int errcode = json.get("errcode").getAsInt();
                                if (0 == errcode) {
                                    Toast.makeText(QuestionDetailActivity.this, "userID或token为空", Toast.LENGTH_SHORT).show();
                                } else if (1 == errcode) {
                                    Toast.makeText(QuestionDetailActivity.this, "数据库查询失败", Toast.LENGTH_SHORT).show();
                                } else if (2 == errcode) {
                                    Toast.makeText(QuestionDetailActivity.this, "用户不存在", Toast.LENGTH_SHORT).show();
                                } else if (3 == errcode) {
                                    Toast.makeText(QuestionDetailActivity.this, "提问保存出错", Toast.LENGTH_SHORT).show();
                                }
                            }
                    ...
                    public void onFailed(Throwable throwable) {//请求失败的时候会调用这个方法
						Toast.makeText(QuestionDetailActivity.this, throwable.getStackTrace().toString(), Toast.LENGTH_SHORT).show();
                    }
                });
    }
