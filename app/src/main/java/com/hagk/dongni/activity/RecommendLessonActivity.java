 ...
 * 添加联系人的界面
 */
public class RecommendLessonActivity extends Activity implements TopBarView.onTitleBarClickListener,View.OnClickListener{
    ...
    public void saveContact() {
        ...
        if (TextUtils.isEmpty(nicknameStr) || TextUtils.isEmpty(phoneStr)) {
            Toast.makeText(RecommendLessonActivity.this, ConstantValue.TXT_EMPTY,
                    Toast.LENGTH_SHORT).show();
            return;
        }

        if (!OthersUtils.isMobileNO(phoneStr)) {
            Toast.makeText(RecommendLessonActivity.this,
        ...
        if("addContact".equals(type)){
            //判断联系人昵称是否存在
            Set<String> contacts = PrefUtils.getWatchContact(RecommendLessonActivity.this); //获取到所有的联系人
            if(contacts != null){
                for(String item : contacts){
                    String temp = item.split(" ")[0];
                    if(nicknameStr.equals(temp)){
                        //如果包含了这个联系人
                        Toast.makeText(RecommendLessonActivity.this,"已经添加过该联系人",Toast.LENGTH_SHORT).show();
                        return;
                    }
                }
            }
        ...
        //如果没有包含该对象,则添加到本地数据库
        PrefUtils.setWatchContact(RecommendLessonActivity.this,nicknameStr + " " +phoneStr);
...
//        postData(nicknameStr, phoneStr);
    ...
    public void postData(final String nicknameStr, final String phoneStr) {
        ...
        String uerID = PrefUtils.getUserID(RecommendLessonActivity.this.getBaseContext());
        String token = PrefUtils.getToken(RecommendLessonActivity.this.getBaseContext());
        ...
        MyHttpUtils.build()//构建myhttputils
                ...
                .onExecuteByPost(new StringCallBack() {//开始执行，并有一个回调（异步的哦---->直接可以更新ui）
                    ...
                    public void onSucceed(String result) {//请求成功之后会调用这个方法----显示结果
                        ...
                        try {
                            ...
                            if (ConstantValue.SUCCESS_STATUS.equals(status)) {
                                ...
                                if (3 == errcode) { //数据库出错
                                    Toast.makeText(RecommendLessonActivity.this, "服务器保存时有空值", Toast.LENGTH_SHORT).show();
                                } else if (5 == errcode) { //给手表发送命令失败
                                    Toast.makeText(RecommendLessonActivity.this, "数据库查询出错", Toast.LENGTH_SHORT).show();
                                } else if (4 == errcode) { //给手表发送命令失败
                                    Toast.makeText(RecommendLessonActivity.this, "该用户注册手机号未绑定手机", Toast.LENGTH_SHORT).show();
                                } else if (7 == errcode) { //给手表发送命令失败
                                    Toast.makeText(RecommendLessonActivity.this, "与手表通信失败", Toast.LENGTH_SHORT).show();
                                } else if (2 == errcode) {
                                    Toast.makeText(RecommendLessonActivity.this, "用户不存在", Toast.LENGTH_SHORT).show();
                                }
                            }
                    ...
                    public void onFailed(Throwable throwable) {//请求失败的时候会调用这个方法
						Toast.makeText(RecommendLessonActivity.this, throwable.getStackTrace().toString(), Toast.LENGTH_SHORT).show();
                    }
                });
    }
