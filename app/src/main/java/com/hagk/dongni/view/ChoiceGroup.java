...
public class ChoiceGroup extends LinearLayout {
    ...
    public void setInitChecked(int index){
        ((QuestionRadio)map.get(index)).setTouch(2);
        setCurrentValue(((QuestionRadio)map.get(index)).getText().toString());
    }
    ...
    public void setView(final Context context){
        ...
        for(int i=0;i<row;i++){
            ...
            for(int j=0;j<column;j++){
                final QuestionRadio button = new QuestionRadio(context);
    ...
    private void clearSelected(int Index){
        ...
        for(int index = 0;index < map.size(); index ++){
            ...
            ((QuestionRadio)map.get(index)).setTouch(1);
        }
    }
